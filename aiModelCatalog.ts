/**
 * Shared AI model list (used by Vite client and Node server bundle).
 * Keep provider ids aligned with server ALLOWED_PROVIDERS and Firestore rules.
 */
...
export function resolveProviderForModel(modelId: string): (typeof AI_MODEL_OPTIONS)[number]["provider"] | null {
  const hit = AI_MODEL_OPTIONS.find((m) => m.id === modelId);
